document.addEventListener('DOMContentLoaded', function() {
    ...
    for (let i = 0; i < 200; i++) {
        ...
        // Random movement
        const tx = (Math.random() > 0.5 ? '' : '-') + (20 + Math.random() * 30) + 'px';
        const ty = (Math.random() > 0.5 ? '' : '-') + (20 + Math.random() * 30) + 'px';
        particle.style.setProperty('--tx', tx);
        particle.style.setProperty('--ty', ty);
        
        // Animation
        particle.style.animation = `moveParticle ${3 + Math.random() * 4}s linear infinite`;
